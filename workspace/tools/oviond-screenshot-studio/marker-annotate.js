...
function markerSvg(w,h,items,redactions=[]){
  ...
  for (const r of redactions) {
    s+=`<rect x="${r.x}" y="${r.y}" width="${r.w}" height="${r.h}" rx="${r.rx||6}" fill="${r.fill||'#FFFFFF'}"/>`;
    if (r.text) s+=`<text x="${r.textX||r.x+8}" y="${r.textY||r.y+18}" font-family="Inter,Arial,sans-serif" font-size="${r.fontSize||13}" font-weight="${r.fontWeight||650}" fill="${r.color||'#0B1020'}">${esc(r.text)}</text>`;
    if (r.subtext) s+=`<text x="${r.subtextX||r.x+8}" y="${r.subtextY||r.y+36}" font-family="Inter,Arial,sans-serif" font-size="${r.subFontSize||12}" font-weight="${r.subFontWeight||500}" fill="${r.subColor||'#64748B'}">${esc(r.subtext)}</text>`;
  }
...
async function main(){
  ...
  await img.composite([{input:markerSvg(meta.width,meta.height,cfg.items,cfg.redactions||[]),top:0,left:0}]).png().toFile(cfg.output);
