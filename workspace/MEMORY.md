...
- Google Ads production access is still blocked, but the correct path is now confirmed: service-account-first, not Workspace-style delegation. On 2026-03-25 the Google Ads skill was rebuilt, the service account and Google Ads API enablement were verified, and live calls still returned `PROJECT_DISABLED` for Cloud project `480335022137`; a Google support ticket is pending before Ads-side access can be finished. On 2026-04-03 the auth doctor was expanded with Cloud-project diagnostics and explicit OAuth refresh-token fallback support; live checks confirmed that service-account JWT minting works, domain-wide-delegation-style Ads minting for `nicole@oviond.com` does not (`unauthorized_client`), and the current runtime lacks the project IAM needed to inspect or enable `googleads.googleapis.com` directly.
