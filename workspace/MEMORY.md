...
- On 2026-05-05 Chris confirmed Nicole will eventually get whatever Supabase access is needed after launch, and that Stripe statuses are already locked into Supabase so customer data there should be accurate and ready for use. Nicole's current recommendation is to keep Stripe as finance-grade reconciliation/fallback initially while making Supabase the operational customer/product truth layer; only reduce direct Stripe dependency after Supabase billing mirrors reconcile cleanly for MRR, trials, past_due, cancellations, plan mix, and invoice status.
- Supabase growth-machine thesis from 2026-05-05 research: build a read-only Nicole Growth Intelligence Layer first, using Supabase/Postgres + the Oviond REST API to expose canonical views for active trials, trials expiring soon, activation milestones, Stripe/billing status, product usage, data-source/integration health, automation/reporting activity, churn risk, and lifecycle triggers. Then pipe clean events into Sequenzy, combine Gleap support signal, and eventually support AI-agent workflows over account health and reporting operations.
