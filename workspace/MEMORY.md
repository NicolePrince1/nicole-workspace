...
- AI/MCP direction: Oviond should be genuinely AI-first through first-class API and MCP, not a bolted-on chatbot. Keep UI stupid simple; let agent-native work happen in ChatGPT, Claude, Codex, Claude Desktop, and whichever AI tools agencies already use. Lead marketing with clarity/simple agency reporting, but keep AI/API/MCP near the top as a major proof point and product priority — not buried as a footnote. Key market message: use Oviond from ChatGPT/Claude/your preferred AI assistant without being locked into one AI provider.
